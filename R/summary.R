...
#' @importFrom dplyr group_by summarise slice_max slice_min filter mutate arrange desc
#' @export
create_management_summary <- function(df, recent_days = 30) {
  ...
  best_store  <- dplyr::slice_max(by_store, total, n = 1)
  worst_store <- dplyr::slice_min(by_store, total, n = 1)
  ...
  cat_growth <- cat_recent %>%
    ...
    dplyr::mutate(
      pct_change = (total - prev_total) / prev_total
    ) %>%
    dplyr::arrange(dplyr::desc(pct_change))

  fastest_growth <- dplyr::slice_max(cat_growth, pct_change, n = 1)
  biggest_drop   <- dplyr::slice_min(cat_growth, pct_change, n = 1)
